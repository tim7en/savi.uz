"""Both sides of the moving average, and both sides of the market.
...
The short side has only ever been measured unconditionally here: longs made
+1,682R against shorts' -477R over 2017-2026, which is why shorts were dropped.
That is not a test of shorting *below a falling average*, which is a different
proposition. This runs both sides under the regime label, so the question is
whether shorts pay when they are confined to the half of the tape that trends
down.

Arms, with the reversal doing the deciding as always:

* long only, full size -- the banked book;
* long only with the regime tilt -- the rule that already survived;
* long and short, no regime -- the unconditional short case, expected to lose;
* long and short with the tilt -- full long above the average and half below,
  full short below and half above, which is the proposal in full;
* the tilt reversed -- if it performs as well, the label carries nothing.
...
def main(argv=None) -> int:
    ...
    for sessions in args.ma_sessions:
        ...
        arms = {
            ...
            f"regime tilt ({low:g}x against the trend)": tilt,
            "regime tilt reversed": lambda t: tilt(t, reverse=True),
        }
