...
def size_function(gex_available, driver_map, shuffled):
    ...
    def size(variant, trade):
        ...
        elif variant == "Both indexes negative half-risk":
...
def main(argv=None):
    ...
    risk_size = size_function(
        gex_available, driver_map, shuffled_regimes(gex_available, calendar)
    )
    ...
    paired_csv = args.outdir / "turtle_gex_macro_paired.csv"
    with paired_csv.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(paired_rows[0]))
        writer.writeheader()
        writer.writerows(paired_rows)
    mapping_csv = args.outdir / "turtle_gex_macro_drivers.csv"
    ...
    report = args.outdir / "turtle_gex_macro.md"
    lines = [
