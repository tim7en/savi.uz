"""Moving-average crossovers as the regime label, against price-versus-average.

Everything tested so far used price against its own average: is the close above
MA(200). A crossover is a different object -- MA(5) above MA(15) asks whether
the recent average is rising through the older one, which can flip while price
stays on the same side of the slow average. It is a rate-of-change test rather
than a position test, and it has not been run.

Both are tilts, never gates, because the gate version of a higher-timeframe
trend filter was already rejected for removing +1,205R of winning longs against
only -281R of losing shorts. Half size against the regime, full size with it.

The reversal decides, as always. If halving against the crossover performs no
better than halving with it, the label carries nothing -- which is exactly how
relative-strength ranking failed after looking promising at one lookback.
"""
...
def parse_args(argv=None):
    ...
    parser.add_argument("--crossovers", nargs="+", default=("5:15", "15:30", "50:200"),
                        help="fast:slow moving-average pairs, in bars")
...
def crossover(bars: list[Bar], fast: int, slow: int) -> dict[str, bool]:
    """Is the fast mean above the slow one, read at the bar before entry?"""
    ...
    for index, close in enumerate(closes):
        ...
        if index >= slow and index + 1 < len(bars):
            out[bars[index + 1].timestamp] = (fast_sum / fast) > (slow_sum / slow)
    return out
...
def main(argv=None) -> int:
    ...
    for sessions in args.ma_sessions:
        ...
        crosses = {}
        for pair in args.crossovers:
            fast, slow = (int(x) for x in pair.split(":"))
            table: dict[tuple[str, str], bool] = {}
            for ticker, bars in book.items():
                for stamp, flag in crossover(bars, fast, slow).items():
                    table[(ticker, stamp)] = flag
            crosses[pair] = table
        ...
        def cross_tilt(pair, reverse=False):
            table = crosses[pair]
            def weight(trade):
                up = table.get((trade["ticker"], trade["entry"]), True)
                if reverse:
                    up = not up
                if trade["dir"] > 0:
                    return 1.0 if up else low
                return low if up else 1.0
            return weight

        arms = {"full size always": lambda t: 1.0,
                f"price vs MA({sessions}) tilt": tilt}
        for pair in args.crossovers:
            arms[f"crossover {pair} tilt"] = cross_tilt(pair)
            arms[f"crossover {pair} REVERSED"] = cross_tilt(pair, reverse=True)
